use order_book::{order::Order, order_book::OrderBook, MarketOrderSide, OrderSide};
...
fn main() {
    ...
    let order = Order::new(OrderSide::Bid, 2.6);
    orderbook.add_limit_order(100.05, order);
    let order = Order::new(OrderSide::Bid, 3.0);
    orderbook.add_limit_order(99.5, order);
    let order = Order::new(OrderSide::Ask, 2.33);
    orderbook.add_limit_order(100.35, order);
    let order = Order::new(OrderSide::Ask, 2.33);
    orderbook.add_limit_order(100.15, order);
